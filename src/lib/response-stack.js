...
function responseStack() {
  ...
  self.update = function (updatedResponse) {
    const existing = hash[updatedResponse.uid];
    ...
    logger.debug(`found ${updatedResponse.uid}`);
  ...
  self.push = function (stubbedResponse) {
    ...
    logger.info(`added ${stubbedResponse.uid}`);
  };
  ...
  self.use = function (method, url) {
    ...
    for (let i = stack.length - 1; i >= 0; i--) {
      ...
      const matchResult = stub.isMatch(method, url);
      if (matchResult) {
        ...
        response = stub.interpolate(matchResult);
        break;
      }
    }
